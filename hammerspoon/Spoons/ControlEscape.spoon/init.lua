...
function obj:init()
  ...
  self.CANCEL_DELAY_SECONDS = 0.150
...
--- ControlEscape:setCancelDelay(seconds)
--- Method
--- Set the key press delay time (in seconds)
--- Parameters:
---  * seconds - A number specifying the delay in seconds
function obj:setCancelDelay(seconds)
  self.CANCEL_DELAY_SECONDS = seconds
  -- Recreate timer with new delay value
  self.controlKeyTimer = hs.timer.delayed.new(self.CANCEL_DELAY_SECONDS, function()
    self.sendEscape = false
  end)
end
